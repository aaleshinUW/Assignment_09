...
# aaleshin, 2022-Sep-04, Modified Code to complete TODOs
#------------------------------------------#
...
class Track():
    ...
    """
    # TODne add Track class code
    # -- Constructor -- #
    def __init__(self, p, t, l):
        # -- Attributes -- #
        try:    
            self.__position = int(p)
            self.__title = str(t)
            self.__length = str(l)
        except Exception:
    ...
    def position(self, value):
        if str(value).isnumeric():
            if value < 1:
                raise Exception('The Position has to be greater than 0!')
    ...
    def length(self, value):
        if str(value).isnumeric():
            raise Exception('The Length has to be a String!')
        else:
            self.__length = value  
